...
async function analyse(section, periodStart) {
	const result = {
		missing: 0,
		unaligned: 0,
		untrusted: 0,
	...
	const lq = 'SELECT * FROM DATA WHERE section = $1 ORDER BY at DESC LIMIT 1';
	const lastLine = await db.pool.query(lq, [section]);
	...
	for (let chunkLast, chunk = periodStartMs; chunk <= dataLast; chunk=chunkLast+PERIOD_LEN_MS) {
		...
		const subq = 'SELECT * FROM data WHERE section = $1 AND dt >= $2 AND dt <= $3';
		const res = await db.pool.query({
			text: `SELECT * FROM (${subq} ORDER BY ID ASC LIMIT $4) as t ORDER BY dt`,
			values: [section, new Date(chunk), new Date(chunkLast), CHUNK_SIZE],
			// rowMode: 'array'
		});
